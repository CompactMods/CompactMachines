...
import org.dave.compactmachines3.init.Potionss;
...
public class MiniaturizationEvents {
    ...
    public static void onPlayerTick_CheckOffhand(TickEvent.PlayerTickEvent event) {
        ...
        if(offHandItemStack.isEmpty() || offHandItemStack.getItem() != Itemss.psd) {
            return;
        }
    ...
    public static void onLivingJump(LivingEvent.LivingJumpEvent event) {
        ...
        double scale = scaleAttribute.getAttributeValue();
        if(entity.motionY >= 0) {
            entity.motionY *= MathHelper.clamp(scale*2d, 0.50f, 1.0f);
        }
    }
