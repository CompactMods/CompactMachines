package dev.compactmods.machines.test;

public final class TestBatches {

    public static final String TUNNELS = "dev/compactmods/machines/api/tunnels";
    public static final String TUNNEL_DATA = "tunnel_data";

    public static final String ROOM_GENERATION = "room_generation";

    public static final String MACHINE_GRAPH = "machine_graph_data";
    public static final String CODEC_TESTS = "codecs";
    public static final String MATH = "math";
    public static final String MACHINE_DATA = "machine_data";
    public static final String DIMENSION = "dev/compactmods/machines/api/dimension";
    public static final String MIGRATION = "migration";
}
